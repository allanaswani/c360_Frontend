    ...
    <div className={s.userWrap} onMouseLeave={() => setOpen(false)}>
      ...
      {open && (
        <div className={s.userMenu} role="menu">
          ...
          {user.is_admin && (
            <Link href="/admin/observability" className={s.userMenuItem} role="menuitem" onClick={() => setOpen(false)}>
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 3v18h18" /><path d="M7 14l3-4 3 3 4-6" /></svg>
              Monitoring
            </Link>
          )}
          {user.is_admin && (
            <Link href="/admin/audit" className={s.userMenuItem} role="menuitem" onClick={() => setOpen(false)}>
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><path d="M14 2v6h6" /><path d="M8 13h8M8 17h5" /></svg>
              Audit trail
            </Link>
          )}
          {user.is_admin && (
